...
function create(category, item, callback){
  if(!cache[category]) return callback(`DataSource Create Error: Category ${category} does not exist`);
  var id = uuid();
  item.id = id;
  //console.log(item);
...
if(cache && cache[parts[0]]) {
  ...
  callback(`DataSource Read Error: Category ${id} does not exist in the data source`);
}
      break;
    case 2:
      // TODO: Serve ONE item
  ...
  // But if it reaches here, then it does not
          callback(`DataSource Read Error: Item ${id} does not exist in the data store`);
          }
      break;
    default:
...
function saveCache(category) {
  try {
    ...
    fs.writeFile(`data/${category}.json`, json, {
      encoding: "utf-8"
    }, function(err) {
